import { pickValidDiscoveryPhoto, isValidDiscoveryPhoto } from "./discoveryPhoto";
...
function findAnalysisRow(rows, analysisId) {
  if (analysisId == null) return null;
  const wanted = String(analysisId);
  return rows.find((row) => row?.id != null && String(row.id) === wanted) || null;
}

/** Fusionne ligne cloud + trouvaille locale (photo/nom) pour l'affichage picker. */
export function mergePickerDiscovery(localDiscovery, row) {
  const cloud = rowToDiscovery(row);
  const id = localDiscovery?.id || row?.client_id || cloud.id;

  return {
    ...cloud,
    id,
    nom: cloud.nom || localDiscovery?.nom || "",
    nom_latin: cloud.nom_latin || localDiscovery?.nom_latin || "",
    type: cloud.type || localDiscovery?.type || "plante",
    photo: pickValidDiscoveryPhoto(
      localDiscovery?.photo,
      localDiscovery?.cloudImageUrl,
      row?.image_url,
      cloud.photo
    ),
    cloudImageUrl: row?.image_url || localDiscovery?.cloudImageUrl || cloud.cloudImageUrl || null,
  };
}

export async function resolveDiscoveryCloudImageUrl(discovery, userId) {
  const existingCloud = discovery.cloudImageUrl || null;
  if (isValidDiscoveryPhoto(existingCloud) && existingCloud.startsWith("http")) {
    return existingCloud;
  }

  const localPhoto = pickValidDiscoveryPhoto(discovery.photo, discovery.cloudImageUrl);
  if (!localPhoto) return null;

  if (localPhoto.startsWith("data:")) {
    return uploadDiscoveryPhoto(userId, discovery.id, localPhoto);
  }
...
export function rowToDiscovery(row) {
  const result = parseAnalysisResult(row.result);
  const discoveryId = row.client_id || result.id || (row.id != null ? String(row.id) : null);
  return {
    ...result,
    id: discoveryId,
...
export async function fetchPlantAnalysesForPalette() {
  ...
  for (const discovery of discoveries) {
    ...
    const row = analysisId != null ? findAnalysisRow(rows, analysisId) : null;
    items.push({
      analysisId: analysisId ?? null,
      discovery: row ? mergePickerDiscovery(discovery, row) : discovery,
    });
  }

  return { ok: true, data: dedupePaletteCandidates(items) };
}

function discoveryRichness(discovery) {
  if (!discovery) return 0;
  let score = 0;
  if (discovery.nom) score += 2;
  if (discovery.nom_latin) score += 1;
  if (pickValidDiscoveryPhoto(discovery.photo, discovery.cloudImageUrl, discovery.image_url)) {
    score += 3;
  }
  if (discovery.id && !/^\d+$/.test(String(discovery.id))) score += 1;
  return score;
}

/** Évite deux lignes picker pour la même analyse (UUID local + id bigint fantôme). */
function dedupePaletteCandidates(items) {
  ...
  for (const item of items) {
    ...
    const key = String(item.analysisId);
    const prev = byAnalysisId.get(key);
    if (!prev || discoveryRichness(item.discovery) > discoveryRichness(prev.discovery)) {
      byAnalysisId.set(key, item);
    }
  }

  const byLocalId = new Map();
  for (const item of localOnly) {
    const key = String(item.discovery?.id ?? "");
    if (!key) continue;
    const prev = byLocalId.get(key);
    if (!prev || discoveryRichness(item.discovery) > discoveryRichness(prev.discovery)) {
      byLocalId.set(key, item);
    }
  }

  return [...byAnalysisId.values(), ...byLocalId.values()];
}
