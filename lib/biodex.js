/** Collection Biodex — espèces découvertes vs silhouettes à compléter. */

export const BIODEX_TYPES = [
  { id: "plante", emoji: "🌿" },
  { id: "fleur", emoji: "🌸" },
  { id: "arbre", emoji: "🌳" },
  { id: "fruit", emoji: "🍎" },
...
export function buildBiodexCollection(discoveries) {
  ...
  const silhouettes = BIODEX_TYPES.filter((t) => !caughtTypes.has(t.id)).map((t) => ({
    id: `silhouette-${t.id}`,
    type: t.id,
    emoji: t.emoji,
    caught: false,
  ...
  return {
    ...
    totalTypes: BIODEX_TYPES.length,
    completionPct: Math.round((caughtTypes.size / BIODEX_TYPES.length) * 100),
  };
}
