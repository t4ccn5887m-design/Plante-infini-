...
export async function pushDiscoveryToCloud(discovery) {
  ...
  if (!auth.ok) {
    return { ok: false, error: auth.error, localOnly: true };
  }

  if (!auth.isPermanent) {
    return { ok: false, skipped: true, localOnly: true, reason: "guest_local_only" };
  }
...
export function scheduleDiscoverySync(discovery) {
  ...
  getCloudSession()
    .then((session) => {
      if (!isPermanentAuthUser(session?.user)) return;
      return pushDiscoveryToCloud(discovery);
    })
    .catch(() => {
      addPendingId(discovery.id);
    });
...
export async function signInWithOAuth(provider, options = {}) {
  ...
  if (options.pendingCheckoutPlan) {
    const { savePendingCheckoutPlan } = await import("@/lib/subscribeFlow");
    savePendingCheckoutPlan(options.pendingCheckoutPlan);
  }
