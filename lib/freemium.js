...
export const PREMIUM_SINCE_KEY = "wilder-premium-since";
...
export function completePremiumActivation() {
  ...
  localStorage.setItem(PREMIUM_PLAN_KEY, plan);
  if (!localStorage.getItem(PREMIUM_SINCE_KEY)) {
    localStorage.setItem(PREMIUM_SINCE_KEY, new Date().toISOString());
  }
...
export function getPremiumRenewalDate() {
  ...
  const sinceRaw = localStorage.getItem(PREMIUM_SINCE_KEY);
  if (!sinceRaw) return null;
  const since = new Date(sinceRaw);
  if (Number.isNaN(since.getTime())) return null;
  const renewal = new Date(since);
  if (getPremiumPlan() === "yearly") {
...
export function cancelPremiumSubscription() {
  if (typeof window === "undefined") return;
  localStorage.removeItem(PREMIUM_KEY);
  localStorage.removeItem(PREMIUM_PLAN_KEY);
  localStorage.removeItem(PREMIUM_SINCE_KEY);
