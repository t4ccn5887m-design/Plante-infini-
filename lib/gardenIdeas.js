...
export const GARDEN_IDEAS = [
  ...
  },
  {
    ...
    items: [
      ...
      { kind: "deco", ref_id: "lanterne-pierre" },
    ],
  },
  {
    id: "cottage-anglais",
...
export function resolveAmbianceItems(ambiance) {
  ...
  for (const item of ambiance?.items || []) {
    ...
    } else if (item.kind === "deco") {
      const article = decoById.get(item.ref_id);
      if (article) deco.push(article);
      else missing.push(item);
    } else {
      missing.push(item);
    }
  ...
  return { vegetal, mineral, deco, missing };
}
...
export function countAmbianceItems(ambiance) {
  ...
  for (const item of ambiance?.items || []) {
    ...
    else if (item.kind === "deco") decoItems += 1;
  }

  return { plants, minerals, deco: decoItems };
}

export function isAmbianceComplete(ambiance, inGardenPlants, inGardenMinerals, inGardenDeco = new Set()) {
  for (const item of ambiance?.items || []) {
    ...
    if (item.kind === "deco" && !inGardenDeco.has(item.ref_id)) return false;
  }
