...
export async function deliverPalettePdf(pdfDoc, filename) {
  ...
  if (typeof navigator.share === "function" && navigator.canShare?.({ files: [file] })) {
    try {
      ...
      if (error?.name === "AbortError") {
        return { ok: false, error: "share_cancelled", cancelled: true };
      }
      console.warn("[Wilder] deliverPalettePdf share failed, fallback download:", error);
    }
  }
...
export function canSharePalettePdf() {
...
export async function downloadPalettePdf({ palette, zones, items, contact, t }) {
  ...
  try {
    ...
    return await deliverPalettePdf(pdfDoc, filename);
  } catch (error) {
    console.error("[Wilder] downloadPalettePdf:", error);
    return { ok: false, error: error.message || "pdf_error" };
  }
}
