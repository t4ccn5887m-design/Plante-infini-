import { rowToDiscovery, verifyAnalysisRowId } from "./analysesStorage";
import { ensureDiscoveryAnalysisRowId } from "./cloudSync";
...
const ANALYSIS_JOIN = "id, client_id, result, image_url, created_at";

function mapJoinedAnalysis(analysisRow) {
  if (!analysisRow) return null;
  return rowToDiscovery(analysisRow);
}
...
export async function fetchPaletteItems(paletteId) {
  ...
  const { data, error } = await supabase
    .from("palette_items")
    .select(`${ITEM_COLUMNS}, analyses ( ${ANALYSIS_JOIN} )`)
    .in("zone_id", zoneIds)
    .order("ordre", { ascending: true });
...
export async function addZoneItems(zoneId, items = []) {
  ...
  for (const item of items) {
    let analysisId = item.analysisId;

    if (analysisId == null) {
      if (!item.discoveryId) {
        return { ok: false, error: "invalid_input", data: [] };
      }
      const ensured = await ensureDiscoveryAnalysisRowId(item.discoveryId);
      if (ensured.rowId == null) {
        return { ok: false, error: ensured.error || "analysis_not_found", data: [] };
      }
      analysisId = ensured.rowId;
    }

    const verified = await verifyAnalysisRowId(userId, analysisId);
    if (!verified.ok) {
