import { verifyAnalysisRowId, resolveAnalysisRowId } from "./analysesStorage";
...
export async function addZoneItems(zoneId, items = []) {
  ...
  for (const item of items) {
    ...
    if (analysisId == null && item.discoveryId) {
      const resolved = await resolveAnalysisRowId(userId, item.discoveryId);
