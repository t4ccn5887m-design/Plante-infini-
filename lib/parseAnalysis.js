import { extractPaysagisteBlock, normalizePaysagisteFields } from "./paysagisteSpecs";
...
/**
 * Parse la réponse Claude complète : JSON principal + bloc paysagiste balisé en fin.
 * Le bloc est extrait avant le parse JSON pour ne pas casser l'identification.
 */
export function parseAnalysisResponse(raw) {
  const { mainText, paysagiste } = extractPaysagisteBlock(raw);
  const parsed = parseClaudeJson(mainText);
  if (!parsed || parsed.erreur) return parsed;
  if (!paysagiste) return parsed;
  return { ...parsed, ...paysagiste };
}
...
function normalizeAnalysis(data) {
  ...
  return {
    ...
    ...(normalizePaysagisteFields(data) || {}),
  };
}
