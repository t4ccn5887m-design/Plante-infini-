...
export async function requestAllPermissions() {
  const cameraPromise = isCameraGranted()
    ? Promise.resolve({ ok: true })
    : requestCameraPermission();
  const cached = loadStoredLocation();
  const locationPromise = cached
    ? Promise.resolve({ ok: true, location: toLocationPayload(cached) })
    : requestLocationPermission();

  const [camera, location] = await Promise.all([cameraPromise, locationPromise]);
  completeOnboarding();
