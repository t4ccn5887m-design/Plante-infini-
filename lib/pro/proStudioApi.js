 ...
 * Cœur = pro_links + pro_briefs (obligatoire).
 * RDV = requête séparée sur pro_appointments (optionnelle — dégradation propre).
 * @returns {{ ok: true, items: ProBriefCard[], appointmentsDegraded?: boolean }
...
export async function listProLinksWithBriefs(studioId) {
  ...
  try {
    ...
    if (error) {
      ...
      return {
        ok: false,
        error: "links_load_failed",
        detail: error.message || String(error),
      };
    }

    const rows = data || [];
    /** @type {Map<string, { id: string, starts_at: string }[]>} */
    const appointmentsByLink = new Map();
    let appointmentsDegraded = false;

    if (rows.length > 0) {
      const { data: appts, error: apptErr } = await supabase
        .from("pro_appointments")
        .select("id, link_id, starts_at")
        .eq("studio_id", studioId);

      if (apptErr) {
        console.warn(
          "[Wilder Pro] appointments enrichment skipped:",
          apptErr.message
        );
        appointmentsDegraded = true;
      } else {
        for (const a of appts || []) {
          if (!a?.link_id) continue;
          const list = appointmentsByLink.get(a.link_id) || [];
          list.push(a);
          appointmentsByLink.set(a.link_id, list);
        }
      }
    }
    ...
    const items = rows.map((row) => {
      ...
      const appointments = pickAppointments(appointmentsByLink.get(row.id));
    ...
    return { ok: true, items, appointmentsDegraded };
  } catch (e) {
 ...
 * Cœur = pro_appointments. Enrichissement noms clients via pro_links (séparé).
 * @returns {{ ok: true, groups, items } | { ok: false, error: string, detail?: string }}
 */
export async function listProAppointments(studioId) {
  ...
  try {
    ...
    if (error) {
      ...
      return {
        ok: false,
        error: "appointments_load_failed",
        detail: error.message || String(error),
      };
    }

    const rows = data || [];
    /** @type {Record<string, { client_name?: string, client_phone?: string, client_address?: string }>} */
    const linksById = {};
    const linkIds = [
      ...new Set(rows.map((r) => r.link_id).filter(Boolean)),
    ];
    ...
    if (linkIds.length > 0) {
      ...
      if (linkErr) {
        console.warn(
          "[Wilder Pro] appointment link enrichment skipped:",
          linkErr.message
        );
      } else {
        for (const l of links || []) {
          if (l?.id) linksById[l.id] = l;
        }
      }
    }

    const items = rows.map((row) => {
      const link = linksById[row.link_id] || null;
      return {
        id: row.id,
        linkId: row.link_id,
...
export function proStudioErrorMessage(code, detail) {
  let base;
  switch (code) {
    ...
    case "not_authenticated":
      base = "Connectez-vous pour accéder à Wilder Pro.";
      break;
    case "studio_load_failed":
    case "studio_create_failed":
      base = "Impossible de charger votre studio. Réessayez.";
      break;
    case "studio_missing":
      base = "Studio introuvable. Rechargez la page.";
      break;
    case "studio_name_required":
      base = "Indiquez le nom du studio.";
      break;
    case "studio_update_failed":
      base = "Impossible d’enregistrer les réglages. Réessayez.";
      break;
    case "client_name_required":
      base = "Indiquez le nom du client.";
      break;
    case "link_create_failed":
    case "token_failed":
      base = "Impossible de créer le lien. Réessayez.";
      break;
    case "links_load_failed":
      base = "Impossible de charger vos briefs.";
      break;
    case "link_missing":
      base = "Lien client introuvable.";
      break;
    case "starts_at_required":
      base = "Choisissez une date et une heure.";
      break;
    case "duration_invalid":
      base = "Durée invalide.";
      break;
    case "appointment_create_failed":
      base = "Impossible d’enregistrer le rendez-vous. Réessayez.";
      break;
    case "appointments_load_failed":
      base = "Impossible de charger l’agenda.";
      break;
    default:
