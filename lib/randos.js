...
/** Hike duration from start to end (or now if still active). */
export function computeRandoDurationMs(album) {
  if (!album?.createdAt) return null;
  const endIso = album.endedAt || new Date().toISOString();
  const ms = new Date(endIso) - new Date(album.createdAt);
  return ms > 0 ? ms : null;
}

export function formatRandoDuration(ms, t) {
