...
export async function getScanQuota(identity) {
  ...
  try {
    const data = await callRpc("get_scan_quota", {
...
export async function checkScanAllowed(identity) {
  ...
  try {
    const data = await callRpc("check_scan_allowed", {
      p_visitor_key: identity.visitorKey,
      p_user_id: identity.userId,
      p_limit: FREE_SCAN_LIMIT,
    });
...
export async function incrementScanCountServer(identity) {
  ...
  try {
    const data = await callRpc("increment_scan_count", {
      p_visitor_key: identity.visitorKey,
      p_user_id: identity.userId,
      p_limit: FREE_SCAN_LIMIT,
    });
...
export async function activateScanPremiumServer(identity) {
  if (!isScanQuotaServerAvailable()) {
    return { ok: false, error: "scan_quota_unavailable" };
  }

  try {
    const data = await callRpc("activate_scan_premium", {
      p_visitor_key: identity.visitorKey,
      p_user_id: identity.userId,
    });
    return { ok: true, ...normalizeQuotaPayload(data) };
  } catch (e) {
    console.error("[Wilder] activateScanPremiumServer:", e.message);
    return { ok: false, error: e.message };
  }
}
...
function normalizeQuotaPayload(data) {
  ...
  return { count, limit, isPremium, canScan };
}

export async function enforceScanQuotaForAnalyze(req, res) {
  const identity = await resolveScanIdentity(req);
  if (!identity.ok) {
    res.status(400).json({ erreur: "Identifiant visiteur manquant" });
    return null;
  }

  if (!isScanQuotaServerAvailable()) {
    console.warn("[Wilder] SUPABASE_SERVICE_ROLE_KEY manquant — quota serveur désactivé");
    return { identity, quota: null, skipped: true };
  }

  const check = await checkScanAllowed(identity);
  if (!check.ok) {
    res.status(503).json({ erreur: "Quota scans indisponible" });
    return null;
  }
  ...
  return { identity, quota: check, skipped: false };
}

export async function recordSuccessfulScanServer(identity) {
