...
/** Message unique pour le partage de l'app Wilder. */
export const WILDER_APP_SHARE_TEXT =
  "🌿 « Vous voulez quoi dans votre jardin ? » La question de ton paysagiste à laquelle tu ne sais jamais répondre. Avec Wilder, scanne les plantes qui te plaisent et montre-lui exactement ce que tu veux. 👉\n\nhttps://wilder-nature.com";

/** Texte de partage de l'app — message unique, sans variante. */
export function buildWilderAppShareText() {
  return WILDER_APP_SHARE_TEXT;
}

/** Partage natif de l'app (WhatsApp, SMS, etc.). Retourne "shared", "clipboard" ou null. */
export async function shareWilderApp() {
  const text = WILDER_APP_SHARE_TEXT;
