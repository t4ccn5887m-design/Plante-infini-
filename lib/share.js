...
/** URL publique de l'app (production). */
export function getWilderAppUrl() {
  const fromEnv = process.env.NEXT_PUBLIC_SITE_URL;
  if (fromEnv) return fromEnv.replace(/\/$/, "");
  if (typeof window !== "undefined" && window.location?.origin) {
    return window.location.origin;
  }
  return "https://wilder-nature.com";
}

/** Texte de partage viral de l'app (espèces identifiées ou message par défaut). */
export function buildWilderAppShareText({
  speciesCount = 0,
  isLoggedIn = false,
  useSpeciesLine = false,
} = {}) {
  const url = getWilderAppUrl();
  if (useSpeciesLine || (isLoggedIn && speciesCount > 0)) {
    return `🍃 J'ai trouvé ${speciesCount} espèces autour de chez moi avec Wilder.\nT'en trouves combien toi ?\nL'appli qui transforme chaque balade en chasse aux trésors 🌱\n👇\n${url}`;
  }
  return `🍃 Découvre Wilder, l'appli qui transforme chaque balade en chasse aux trésors 🌱\nTélécharge-la ici 👇\n${url}`;
}

/** Partage natif de l'app (WhatsApp, SMS, etc.). Retourne "shared", "clipboard" ou null. */
export async function shareWilderApp({
  speciesCount = 0,
  isLoggedIn = false,
  useSpeciesLine = false,
} = {}) {
  const text = buildWilderAppShareText({ speciesCount, isLoggedIn, useSpeciesLine });
  const url = getWilderAppUrl();
