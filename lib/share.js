...
export function buildWilderAppShareText({ speciesCount = 0, isLoggedIn = false } = {}) {
  const url = getWilderAppUrl();
  if (isLoggedIn && speciesCount > 0) {
...
/** Partage natif de l'app (WhatsApp, SMS, etc.). */
export async function shareWilderApp({ speciesCount = 0, isLoggedIn = false } = {}) {
  const text = buildWilderAppShareText({ speciesCount, isLoggedIn });
  const url = getWilderAppUrl();
